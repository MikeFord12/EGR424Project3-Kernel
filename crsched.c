                ...
                if (setjmp(scheduler_buf)==0) {
                        ...
                        if (!threads[currThread].active) {
                                /*  int adds=0;
                                   ...
                                   {
                                          if(*(threads[currThread].stack-(count*4))==0xF &&*(threads[currThread].stack-(count*4)+1)==0xF  &&*(threads[currThread].stack-(count*4)+2)==0xF  &&*(threads[currThread].stack-(count*4)+3)==0xF)
                                          {
                                                  adds++;
                                          }

                                   }
                                   iprintf("Free Bytes: %d\n\r",adds);*/
